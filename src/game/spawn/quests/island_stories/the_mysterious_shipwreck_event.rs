...
fn continue_journey(actions: &mut StoryActions) {
    actions.delta_items(Item::Gold, 50);
    actions.add_dialogue(captain!("We decided to press on with our journey. As we sailed away, we spotted something glinting in the water and fished out a small pouch of gold coins!"));
}

pub fn the_mysterious_shipwreck_event(actions: &mut StoryActions) -> DayEvent {
    island_stories_base(actions)
        .line(crew1!("Cap'n! There's a shipwreck on the beach of that island ahead!"))
        .line(captain!("Interesting... Any signs of survivors or other ships in the area?"))
        .line(crew2!("No signs of life, Cap'n. The wreck looks old, might be worth salvaging."))
        .line(crew3!("The island itself looks lush. Could be a good spot to replenish our supplies."))
        .line(captain!("We've got options, crew. What's our move?"))
        .choice("Salvage", salvage_wreck)
        .choice("Explore", explore_island)
        .choice("Sail On", continue_journey)
