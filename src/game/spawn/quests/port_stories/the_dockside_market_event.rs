...
fn purchase_supplies(actions: &mut StoryActions) {
    if actions.get_item(Item::Gold) >= 50 {
        ...
        actions.add_dialogue(captain!("We could only afford some basic supplies, but it's better than nothing."));
    }
}
...
fn sell_excess_cargo(actions: &mut StoryActions) {
    ...
    actions.delta_food(-5);
    actions.add_dialogue(captain!(format!("We sold some of our excess cargo for {} gold. It's freed up some space in our hold.", gold_gain)));
}
...
fn recruit_new_crew(actions: &mut StoryActions) {
    if actions.get_item(Item::Gold) >= 30 {
        ...
        actions.add_dialogue(captain!("We found a capable sailor looking for work. They've joined our crew for a modest fee."));
    } else {
        actions.add_dialogue(captain!("We spotted some potential recruits, but couldn't afford to hire anyone new at the moment."));
    }
}

pub fn the_dockside_market_event(actions: &StoryActions) -> DayEvent {
