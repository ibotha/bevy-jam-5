...
fn restock_supplies(actions: &mut StoryActions) {
    if actions.get_item(Item::Gold) >= 40 {
        ...
        actions.add_dialogue(captain!(
            "Our hold is well-stocked now. We shouldn't go hungry on our next journey."
        ));
    } else {
        actions.delta_items(Item::Gold, -15);
        actions.delta_food(8);
        actions.add_dialogue(captain!(
            "We picked up what supplies we could afford. It's not much, but it'll have to do."
        ));
    }
}
...
pub fn the_harbor_maintenance_day_event(actions: &mut StoryActions) -> DayEvent {
    port_stories_base(actions)
        .line(crew1!(
            "Cap'n, we've got some time in port. The ship could use some attention."
        ))
        .line(captain!(
            "Indeed, a well-maintained ship is crucial. What are our priorities?"
        ))
        .line(crew2!(
            "The hull's taken a beating on our last voyage, and our supplies are running low."
        ))
        .line(crew3!(
            "We could also use this time to train and improve our skills, Cap'n."
        ))
        .line(captain!(
            "All important tasks. Let's decide what to focus on."
        ))
        .conditional_choice(
            "Repair hull",
            repair_hull,
